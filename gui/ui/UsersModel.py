...
class UsersModel(QtCore.QAbstractListModel):
	...
	PwdImgPathsRole = QtCore.Qt.UserRole+104
	MetaInfoRole = QtCore.Qt.UserRole+1005
	...
	def data(self, index, role=QtCore.Qt.DisplayRole):
		...
		if 0 <= index.row() < self.rowCount() and index.isValid():
			...
			if role == UsersModel.UsernameRole:
				...
				return item["surname"]
			elif role == UsersModel.PwdImgPathsRole:
				return item["pwdImgPaths"]
	...
	def roleNames(self):
		...
		roles[UsersModel.PwdImgPathsRole] = b"pwdImgPaths"
	...
	def appendRow(self,username,login,name,surname,pwdImgPaths,metaInfo):
		...
		if username not in tmpId and username !="" and len(tmpN)>0:
			...
			self._entries.append(dict(username=username,login=login,name=name,surname=surname,pwdImgPaths=pwdImgPaths,metaInfo=metaInfo))
			self.endInsertRows()
